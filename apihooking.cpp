...
CHAR patch[5] = {0};//buffer to write the jump offset
...
int __stdcall hook(HWND hWnd,LPCSTR lpText,LPCSTR lpCaption,UINT uType) { //stdcall immitates MessageBoxA stdcall, its also the proxyfunction
	...
	cout <<"Text is " << (LPCSTR)lpText << endl; //printing data from the messageboxa when it was called 
	cout << "Caption is " << (LPCSTR)lpCaption << endl;//printing data from the messageboxa when it was called
